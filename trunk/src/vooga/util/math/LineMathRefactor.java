...
public class LineMathRefactor
{
	...
	private static Line2D rotateAround(Line2D line, Point2D p, double dAngle) {
		Line2D radius = new Line2D.Double(LineMathRefactor.findMidpoint(line), p);
		return LineMathRefactor.moveCenterTo(line, LineMathRefactor.rotate(radius,dAngle).getP2());
	}


	private static Line2D moveCenterTo(Line2D line, Point2D nC) {
		double dx = nC.getX() - LineMathRefactor.findMidpoint(line).getX();
		double dy = nC.getY() - LineMathRefactor.findMidpoint(line).getY();
	...
	public static Line2D rotate(Line2D radius, double dAngle) {
		double mag = LineMathRefactor.length(radius);
		double nA = LineMathRefactor.findDirection(radius)+dAngle;
	...
	public static double length(Point2D p1, Point2D p2) {
		return LineMathRefactor.length(new Line2D.Double(p1,p2));
	}
