...
public class TetrisGame extends Game implements ITetrisContext {
	...
	private void initGameEssentials() {
		...
		matrixBackground = new Sprite(matrixImage, resources
				.getInteger("MatrixSpawnCenterWidth") - 10, 100);

		BufferedImage matrixBorderImage = ImageFactory.drawMatrixBorder(260,
				440);
		matrixBorder = new Sprite(matrixBorderImage, resources
				.getInteger("MatrixSpawnCenterWidth") - 41, 72);
	...
	public void update(long elapsedTime) {
		...
		if (!gameOver) {
			...
			if (blockGravityTimer.action(elapsedTime)) {
				boolean result = activeMino.moveY(-1, tetrisMatrix);
				if (!result) {
					// TODO: This is redundant from
					// HardDropPiece.performKeyPressed
					getActiveMino().hardDrop(getTetrisMatrix());
					getTetrisMatrix().lockMino(getActiveMino(), this);
					generateNextBlock();
				}
			}
		}
