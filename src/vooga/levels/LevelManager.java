 ...
 * progress in the game. Additionally, this provides convenience methods in
 * order to easily modify the current playingfield.
 * 
...
public class LevelManager implements VoogaState {
	 ...
	 * 
	 * @param voogaGame
	 *            whose levels this is managing
	 * @param players
	 *            for this game (persistent)
	 */
	...
	/**
	 * Loads the first level specified in the level order file. This is the same
	 * as calling loadLevel(0)
	 */
	@Deprecated
	public void start() {
		loadLevel(0);
	...
	public void loadLevel(int id) {
		...
		for (AbstractLevel pastLevel : myPastLevels) {
			...
			pastLevelClass = pastLevelClass.substring(0,
					pastLevelClass.indexOf(".")); // Gets rid of ".class"
		...
		// instance
		try {
			myActiveLevel = ((AbstractLevel) Reflection.createInstance(
					LEVEL_CLASS_PATH_PREFIX + desiredLevelType, myPlayers,
					myGame));
		} catch (Exception e) {
	 ...
	 * 
	 * @param archetype
	 *            of the sprite that you wish to create
	 * @param x
	 *            coordinate of the sprite
	 * @param y
	 *            coordinate of the sprite
	 * @param Additional
	 *            components for this sprite
	 * @return the newly created sprite
	 */
	public Sprite addArchetypeSprite(String type, int x, int y,
	 ...
	 * 
	 * @param the
	 *            new level order mapping level number to the associated XML
	 *            file path
	 */
	 ...
	 * 
	 * @param The
	 *            graphics which will be used to render
	 */
