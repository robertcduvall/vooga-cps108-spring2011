 ...
 */
public class RefactoredSprite extends BaseSprite
implements java.io.Serializable, IBuildable, ICollidable, IMobility, ISprite
{
	...
	private boolean boundsViewable = false;
	 ...
	 * @see #setLocation(double, double)
	 */
	public RefactoredSprite (BufferedImage image)
	{
		super(image, 0, 0);
	}
	 ...
	 */
	public RefactoredSprite (BufferedImage image, double x, double y)
	{
		super(image, x, y);
	}
	 ...
	 * @see #setImage(BufferedImage)
	 */
	public RefactoredSprite (double x, double y)
	{
		super(x, y);
	}

	public RefactoredSprite (BufferedImage image, double x, double y, IComponent...comps)
	{
		super(image,x,y);
		this.addComponents(comps);
	}
	public RefactoredSprite (double x, double y, IComponent...comps){
		this(null,x,y, comps);
	}
	...
	public boolean carriesExactComponents (IComponent ... comps)
	{
		for (IComponent c : comps)
		{
			...
			else return !this.getComponent(c.getClass()).equals(c);
		}
		return true;
	}
	...
	public <T extends ICollisionShape> T getCollisionShape ()
	{
		if (!this.carriesComponent(CollisionShapeC.class))
		{
			...
			this.addComponent(new CollisionPolygonC(
					new CollisionPolygon(getQuadrilateral())));
		}

		return (T) this.getComponent(CollisionShapeC.class).getCollisionShape();
	}
	
	private CollisionQuadrilateral getQuadrilateral()
	{
		return new CollisionQuadrilateral(new Vertex(this.getX(),this.getY()),
	...
	public void render (Graphics2D g, int x, int y)
	{
		...
		if(this.boundsViewable)
		{
			AffineTransform aTransform = new AffineTransform();
			aTransform.translate((int) this.getX() +width/2, 
					(int) this.getY()+height/2);
			aTransform.rotate(Math.toRadians(this.getAngle()+getArbitraryRotate()));
	...
	/**
	 * Sets the bounds viewable for a sprite. Default off
	 */
	public void setBoundsViewable(boolean boundsViewable)
	{
		this.boundsViewable = boundsViewable;
	}
