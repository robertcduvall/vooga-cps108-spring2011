...
public class VoogaButton {
    ...
    public boolean hitBy(double mouseX, double mouseY) {
        int xRange = position.width + imageSizes.width;
        int yRange = position.height + imageSizes.height;
        if(mouseX > position.width && mouseX < position.width + xRange)
            if(mouseY > position.height && mouseY < position.height + yRange)
                return true;
        return false;
    }
